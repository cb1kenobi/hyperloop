		...
		if (key === 'fatal') {
			...
			if (shouldLog('debug')) {
				...
				for (var i = 0, iL = arguments.length; i < iL; i++) {
					arguments[i].stack && exports.debug('Trace: Argument ' + i + ':', arguments[i].stack);
				}
...
function exit(statusCode) {
	...
	if (statusCode !== undefined && statusCode !== 0 && !exports.report) {
		exports.info('Hint: If you think you have found a bug, run again with '.grey + '--report'.bold + ' to report it.'.grey);
		!shouldLog('debug') && exports.info('Running with '.grey + '--debug'.bold + ' can also give you more information on what is going wrong.'.grey);
	}
	if (exports.report) {
