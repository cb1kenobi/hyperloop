 ...
 */
SourceFile.prototype.processCast = function(argType, argObject, functionName) {
	this.useJSCast = true;
	this.symbols.push({
		type: 'cast',
		argType: argType,
 ...
 */
var GlobalFunctions = ['require','parseInt','parseFloat','eval','setTimeout','setInterval','this','super','String','Number','__dirname','__filename','undefined','arguments'],
...
SourceFile.prototype.processTransformCompleted = function() {
	// we need to mark the symbols that are inside a try/catch block
	this.symbols.forEach(function(sym){
		sym.insideTryCatch = sym.node.inside_try_catch;
	});
};
