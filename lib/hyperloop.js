...
const RESERVED_SYMBOLS = /(static|native|class|package|import|interface)/;
            ...
            if (node instanceof Uglify.AST_SimpleStatement) {
            	...
            	node.definitions.forEach(function(def){
            		var name = def.name.name,
            			value = def.value;
	            	if (name==='callback') {
	            		if(value.expression && value.expression.name==='interface') {
	            			var callback = toValue(value,node);
	            			if (callback && callback.args && callback.args.length!=1) {
	            				throw new Error('expected interface to only take one argument and received '+callback.args.length+' in '+node.start.file+' at '+node.start.line+':'+node.start.col);
	            			}
	            			var fn = callback.args[0];
		            		// if this looks like a function, awesome.
	            			if (!fn.argnames || !fn.body || !fn.variables) {
	            				throw new Error('expected interface to only take one argument which is a function in '+node.start.file+' at '+node.start.line+':'+node.start.col);
	            			}
	            			value.expression.name = sourcefile.processInterface(node,fn);
	            		}
	            	}
            	});
            }
