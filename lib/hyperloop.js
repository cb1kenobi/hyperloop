...
commands['compile'] = new Command('compile', 'compile source files', function(options,args) {
	...
	function generateASTs(asts) {
		...
		asts.forEach(function(entry) {
			...
			if (entry.ast) {
				compressed = compiler.compress(entry.ast, build_opts, filename);
			...
			if (options.debug) {
				...
				if (compressed) {
					var jssrcMap = path.join(options.dest,'src','_map',jsfilename+'.js.map');
					wrench.mkdirSyncRecursive(path.dirname(jssrcMap));
					fs.writeFileSync(jssrcMap, compressed.map);
					log.debug('wrote output JS surcemap at '+jssrcMap.yellow);
				}
			}
