						...
						fns.forEach(function(symbol){
							...
							if (f.arguments.length!=symbol.args.length) {
								...
								if (/\.\.\.\)$/.test(f.signature)==false) {
									...
									// this is a variadic method so we need to wrap it 
									...
									var mockargs = [],
										mocksig  = [],
										mockvars = [];
									var body = f.returnType+=' (^'+symbol.varargSymbol+')(';
									symbol.args.forEach(function(){
										mocksig.push('id');
										var n = mockargs.length;
										mockargs.push('id arg'+n);
										mockvars.push('arg'+n);
									});
									symbol.varargVars = mockvars.join(',');
									body+=mocksig.join(',')+') = ^('+mockargs.join(',')+'){\n';
									var result = f.name+'('+mockvars.join(',')+');';
									if (!noResult) {
										// TODO: wrap return result
										body+='   id result$ = ';
									}
									else {
										body+='   ';
	...
	switch(gen.ref.metatype) {
		case 'function': {
			...
			if (/\.\.\.\)$/.test(gen.ref.signature)) {
				// this is a vararg, we need to build the signature differently
				var body = [];
				console.log(gen);
			}
			else {
...
ejs.filters.source = function(code, indent) {
	return formatSource(code,indent||'\t');
}
