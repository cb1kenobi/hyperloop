			...
			switch(symbol.type) {
				...
				case 'unknown': {
					...
					switch (symbol.metatype) {
						...
						case 'symbol': {
							// check to see if this is a normal variable defined in scope
							// we must check this first, in case there are symbols that might
							// also have the same name in our metabase
							obj = symbol.node.scope.find_variable(symbol.name);
							if (!obj) {
		...
		buildlib.getXcodeSettings(function(err,settings){
			...
			function executeWithClangAST(ast) {
				...
				var srcs = [],
			   	...
			   	try {
			   		...
			   		jsengine.generate(genopts, executeCodeGen);
			   	}
			   	catch(E) {
			   		log.debug(E.stack)
			   		log.error(E);
			...
			if (fs.existsSync(cacheFile)) {
				...
				try {
					fs.readFile(cacheFile, function(err,buf){
						zlib.gunzip(buf, function(err,buf) {
							var astJSON = JSON.parse(String(buf));
							executeWithClangAST(astJSON);
						});
					});
				}
				catch(E) {
				...
				buildlib.clang(headerfile,minversion,nativeargs,jsengine.arc,function(err,result){
					if (err) return callback(err);
					log.debug('Generated clang AST in',((Date.now()/ts)/1000).toFixed(3),'seconds.');
					ts = Date.now();
					var spinner;
					if (!options.colors && options.colors!==undefined) {
						log.info('Generating system metabase. This will take up to a minute (or greater) dependening on your environment.'.green.bold,'This file will be cached and will execute faster on subsequent builds.'.cyan);
					}
					else {
						spinner = require('../spinner');
						spinner.start('Generating system metabase. This will take up to a minute (or greater) dependening on your environment.'.green.bold+' This file will be cached and will execute faster on subsequent builds.'.cyan);
					}
					clangparser.parseBuffer(result,function(err,ast){
						if (err) return callback(err);
						log.debug('Generating AST cache file at',cacheFile,'in',((Date.now()/ts)/1000).toFixed(3),'seconds');
						var astJSON = ast.toJSON();
						zlib.gzip(JSON.stringify(astJSON,null,3),function(err,buf){
							fs.writeFile(cacheFile,buf,function(){
								executeWithClangAST(astJSON);
								spinner && spinner.stop() && console.log();
							});
						});
					});
				});
