...
function validateSourceAgainstAST(ast, sources, asts) {
					...
					if (m.args.length) {
						for (var c=0;c<m.args.length;c++) {
							...
							if (type==='SEL') {
								...
								console.log('we need to wrap because of type:',type);
								console.log('instance=',instance,'method=',method);
...
iOSCodegen.prototype.generate = function(asts,generateASTCallback,callback) {
	...
	var generate = [],
		...
		prefix = this.options.classprefix || 'HL$',
		sourcefiles = [];
	...
	buildlib.getSystemFrameworks(function(err,frameworks){
		...
		buildlib.getXcodeSettings(function(err,settings){
			...
			var hashkey = objch.sort().join('') + nativeargs.sort().join(''),
				...
				moduleCache = path.join(tmpdir,'cache'),
			...
			function executeWithClangAST(ast) {
				...
				validateSourceAgainstAST(ast, self.sources, asts);
