			...
			function executeWithClangAST(ast) {
				...
				var srcs = [],
			   	...
			   	try {
			   		...
			   		jsengine.generate(ast, codegenerate, depends[0]||'UIApplication', depends, generated, prefix, srchash, srcs, options.srcdir);
			   	}
			   	catch(E) {
			   		log.debug(E.stack)
			   		log.error(E);
					...
					if (gen.cached) {
						...
						tasks.push(function(callback){
							...
							zlib.gzip(sourcecode,function(err,buf){
								...
								jsengine.generateCode(gen,function(err,source){
									if (err) return callback(err);
...
function generateStatic(ast,gen,indent) {
	switch(gen.ref.metatype) {
		case 'function': {
			return jsengine.toFunction(ast,gen,indent);
		}
		case 'constant': {
			return jsengine.toValue(ast,gen.ref,indent);
		}
	}
}
