			...
			switch(symbol.type) {
				...
				case 'method': {
					...
					var method = symbol.method,
						node = symbol.node,
						symbolRef = node.expression.expression.scope && node.expression.expression.scope.find_variable(symbol.property),
					...
					if (!symbolRef) {
						...
						if (symbolRef.init && symbolRef.init.start && symbolRef.init.start.value==='hyperloop_class') {
							return;
						}
						try {
							...
							if (/hard time/.test(ex.message)) {
								//TODO: for now, skip these
								return;
							}
							throw ex;
						}
					...
					var m = findMethod(ast, node, obj, method);
					if (!m) {
						...
						if (staticCls && staticCls.methods) {
							m = findMethod(ast, node, staticCls, method);
...
iOSCodegen.prototype.generate = function(asts,generateASTCallback,callback) {
	...
	buildlib.getSystemFrameworks(function(err,frameworks){
		...
		buildlib.getXcodeSettings(function(err,settings){
			...
			var hashkey = objch.sort().join('') + nativeargs.sort().join('') + jsengine.arc,
