...
iOSCodegen.prototype.generate = function(asts,generateASTCallback,callback) {
	...
	if (options.ticurrent) {
		...
		var escapePaths = function(cmd) {
  			return cmd.replace(/(["\s'$`\\])/g,'\\$1');
		};
		cflags.push('-DUSE_TIJSCORE=1');
		cflags.push('-I'+escapePaths(path.resolve(path.join(tisdkDir,'headers'))));
		cflags.push('-I'+escapePaths(path.resolve(path.join(tisdkDir,'include'))));
		linkflags.push('-L'+escapePaths(path.resolve(tisdkDir)));
