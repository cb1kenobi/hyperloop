 ...
 */
function executeSimulator(name,build_dir,settings,callback,callback_logger) {
...
Packager.prototype.package = function(options,args,callback) {
	...
	options.main = options.classprefix + options.main.replace(/\.hjs/g,'').replace(/\\/,'_');
	    ...
	    if (options.src) {
				...
				copyfiles.forEach(function(f){
					...
					if (isDir) {
						...
						if (!/\.(js|hjs|m|c|cpp|mm|h)$/.test(f)) {
							log.debug('Copying',fp.cyan,'to',dest.cyan);
							copyFile(fp,dest);
						}
					}
				});
			...
			if (options.launch && platform==='simulator') {
				executeSimulator(options.name,appdir,settings,callback,options.logger);
			}
		});
	});
