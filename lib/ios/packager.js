				...
				if (match) {
					...
					switch(label) {
						...
						case 'ERROR': {
							...
							return log.errorNoExit(content);
						}
					}
				}
				else {
...
Packager.prototype.package = function(options,args,callback) {
	...
	copyAndFilterString(prefixTemplate,prefixDest,copyValues);

	copyAndFilterEJS(path.join(templateDir,'HyperloopApp.h'),path.join(xcodeSrcDest,'HyperloopApp.h'),{
	...
	copyAndFilterEJS(path.join(templateDir,'HyperloopApp.m'),path.join(xcodeSrcDest,'HyperloopApp.m'),{
		product_name: safeName,
		bundle_id: options.appid,
		main_js: main_js,
		prefix: options.classprefix,
	...
	copyAndFilterEJS(infoplistTemplate,infoplistDest,{
		product_name: safeName,
		bundle_id: options.appid,
		main_js: main_js
	});
