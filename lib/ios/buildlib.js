		...
		Object.keys(sdks).forEach(function(sdk){
			...
			srcfiles.forEach(function(src){
				...
				var outfile = path.join(dir, basename.replace(/\.m[m]?$/,'.o')),
					...
					crashEnabled = cflags.filter(function(f){ return f.indexOf('-DHL_DISABLE_CRASH')!==-1; }).length===0,
					cflag = crashEnabled ? '-F'+path.join(__dirname,'../../deps/ios') : '',
				...
				if (crashEnabled) {
					...
					var link = null; try { link = fs.readlinkSync(path.join(ks,'Headers')) } catch (E) {}
					if (link!=='Versions/Current/Headers') {
						// make the symlinks necessary for build
						var cwd = process.cwd();
						process.chdir(ks);
						fs.symlinkSync('Versions/Current/Headers','Headers','dir');
						fs.symlinkSync('Versions/Current/KSCrash','KSCrash','dir');
						fs.symlinkSync('Versions/Current/Resources','Resources','dir');
						process.chdir(cwd);
					}
				}
