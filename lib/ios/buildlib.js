...
function checkResults(err,results,callback) {
	// console.log('checkResults called err=',err,'results=',results)
...
function clang(file, minVersion, nativeArgs, arc, callback) {
	getXcodeSettings(function(err,settings){
		if (err) return callback(err);

		var args = getDefaultCompilerArgs(!arc).concat(nativeArgs||[]),
			stdout = '',
			stderr = '';
		...
		var p = spawn(settings.clang, args, {env:process.env});
...
/**
 * compile a minimal iOS app
 */
function compileApp(options,callback) {
	getXcodeSettings(function(err,settings){
		if (err) return callback(err);

		var args = getDefaultCompilerArgs(options.no_arc).concat(options.linkflags||[]),
			stdout = '',
			stderr = '';

		args.push('-arch');
		args.push(options.arch);
		args.push('-mios-'+options.platform+'-version-min='+(options['min-version'] || '7.0'));
		args.push('-isysroot');
		args.push(options.platform==='simulator' ? settings.simSDKPath : settings.deviceSDKPath);
		args.push('-Xlinker');
		args.push('-objc_abi_version');
		args.push('-Xlinker');
		args.push('2');
		args.push('-ObjC');
		args.push('-fobjc-arc');
		...
		args.push('-Xlinker');
		args.push('-no_implicit_dylibs');
		args.push('-framework');
		args.push('UIKit');
		args.push('-framework');
		args.push('JavaScriptCore');
		args.push('-framework');
		args.push('CoreGraphics');
		args.push('-weak_framework');
		args.push('Foundation');
		args.push('-lz');
		args = args.concat(options.objfiles);
		args.push('-o');
