...
Node.prototype.parseObjCMethodDeclAfterChildren = function() {
	...
	if (this.name in this.parent.methods) {
		var a = this.parent.methods[this.name];
		if (a.constructor.name === Array.prototype.constructor.name) {
			a.push(methodObj);
		}
		else {
			this.parent.methods[this.name] = [a, methodObj];
		}
	}
	else {
...
var fwkHeaderRegex = /(\w+)\.framework\/Headers\/(.*)/;
...
Node.prototype.parseObjCPropertyDeclAfterChildren = function() {
	...
	if (fwkHeaderRegex.test(header)) {
		var m = fwkHeaderRegex.exec(header);
		header = m[1]+'/'+m[2];
	}
	else {
		header = null;
...
function astToJSON (ast) {
	...
	categories.forEach(function(node){
			...
			methods && Object.keys(methods).forEach(function(n){
				...
				if (!method.framework) method.framework = node.framework;
				m[n] = method;
...
function parseSystemFrameworks(callback) {
	if (systemHeaders) return callback(null,systemHeaders,systemframeworkDir);
	...
	processSystemFrameworks(function(err,result,frameworks,frameworkDir){
		...
		callback(null, result, frameworkDir);
	});	
}

function parse(stream, pipe, callback) {

	parseSystemFrameworks(function(err,systemHeaders){
		if (err) return callback(err);
		...
		frameworks.forEach(function(f){
			...
			if (fs.existsSync(headersDir)) 
			{
				...
				if (headerfiles.length > 0){
					...
					console.error("Couldn't find",fnp);
				}
			}
			else {
				console.error("Couldn't find",headersDir);
			}
		});
...
if (module.id === ".") {
	// parseSystemFrameworks(function(err,result){
	// 	console.log(result)
	// });
	// return;
	parseFile(path.join(__dirname,'..','..','clangout'),function(err,ast){
		...
		//var j = ast.toJSON();
		//console.log(j.classes.NSCoder.methods.setObjectZone)
