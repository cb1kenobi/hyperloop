...
function internalGenerate (metadata, generation, interfaceName, depends, generated) {
    ...
    entry.makeBinding = codegen.makeBinding;
...
function generate (metadata, generation, interfaceName, depends, generated, imports) {

    var code = generateRecursive(metadata, generation, interfaceName, depends, generated, imports);
    ...
    generated.registerName = 'RegisterTiJSClasses_'+(+new Date);

    code+='/**\n';
    code+=' * called to dynamically add our JSExport protocols to the system interfaces\n';
    code+=' */\n';
    code+='void '+generated.registerName+'()\n';
    code+='{\n';
    ...
    generated.forEach(function(i){
        ...
        if (!generate && (intf.properties && Object.keys(intf.properties).length > 0 || intf.methods && Object.keys(intf.methods).length > 0))
        {
            generate = true;
        }        
        ...
        if (generate) {
                ...
                intf.properties && Object.keys(intf.properties).forEach(function(k){
                    var p = intf.properties[k];
                ...
                if (intf.metatype === 'interface') {
                    intf.methods && Object.keys(intf.methods).forEach(function(k){
                        var m = intf.methods[k],
                            e = makeExport(m);
