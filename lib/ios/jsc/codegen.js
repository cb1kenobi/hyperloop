...
function processSymbols(version, metadata, framework, symbols, dir, srcs) {
	...
	var filename = path.join(dir, 'js_'+framework, framework+'_globals'),
...
function processClass(version, metadata, name, entry, dir, generated, srcs) {
	...
	var filename = path.join(dir, 'js_'+entry.framework, entry.name),
		dirname = path.dirname(filename),
		externs = [],
		includes = [],
		makers = [],
...
function generateCustomClass(metadata,name,classdef,srcdir,srcs,version,generated) {
	...
	classdef.methods.forEach(function(method){
		...
		if (method.arguments) {
			...
			method.body+='\tJSStringRef name$'+first.name+' = JSStringCreateWithUTF8CString("'+(first.property||first.name)+'");\n';
			...
			for (var c=1;c<method.arguments.length;c++) {
				...
				method.body+='\tJSStringRef name$'+arg.name+' = JSStringCreateWithUTF8CString("'+(arg.property||arg.name)+'");\n';
...
function generateCode (gen, genopts, callback) {
	...
	Object.keys(gen.statics).forEach(function(n){
		...
		{
			if (entry.ref.framework && imports.indexOf(entry.ref.framework)===-1) {
