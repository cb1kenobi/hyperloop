...
function generateInvocation(metadata,externs,makers,entry,method,instance,name,indent,skipLengthCheck,imports) {
	...
	if (method.args.length === 0) {
		...
		body.push(indent+generateJSCType(metadata,externs,makers,entry.name,method.returnType,method.returnSubtype,'value','result',indent,imports));
...
//	console.error('gen varname=',varname,'resultname=',resultname)
		...
		case 'interface': {
			...
			body.push(indent + thetype + ' ' +resultname + ' = ('+thetype+') HyperloopGetPrivateObjectAsID('+resultname+'Obj);');
		...
		case 'primitive': {
			if (typeObject.is_pointer) {
				if (typeObject.simple_type==='char') {
					...
					body.push(indent+'{');
					body.push(indent+indent+resultname+'StringRef = JSValueToStringCopy(ctx,'+varname+',0);');
					body.push(indent+indent+'size_t '+resultname+'size = JSStringGetMaximumUTF8CStringSize('+resultname+'StringRef);');
					body.push(indent+indent+resultname+' = malloc(sizeof(char)*'+resultname+'size);');
					body.push(indent+indent+'size_t '+resultname+'len = JSStringGetUTF8CString('+resultname+'StringRef,(char*)'+resultname+','+resultname+'size);');
					body.push(indent+indent+resultname+'['+resultname+"len]='\\0';");
					body.push(indent+'}');
					body.push(indent+'else if (JSValueIsObject(ctx,'+varname+'))');
					body.push(indent+'{');
					body.push(indent+indent+'JSObjectRef '+varname+'Obj = JSValueToObject(ctx,'+varname+',0);');
					body.push(indent+indent+'if (HyperloopPrivateObjectIsType('+varname+'Obj,JSPrivateObjectTypeJSBuffer))');
					body.push(indent+indent+'{');
					body.push(indent+indent+indent+'JSBuffer *buf = HyperloopGetPrivateObjectAsJSBuffer('+varname+'Obj);');
					body.push(indent+indent+indent+resultname+' = (char*)buf->buffer;');
					body.push(indent+indent+indent+resultname+'free = false;'); // don't free it, we're using direct memory buffer
					body.push(indent+indent+'}')
					body.push(indent+'}');
					body.push(indent+'else');
					body.push(indent+'{');
					body.push(indent+indent+makeException(indent,'unsupported char type passed'));
					body.push(indent+'}');
		...
		case 'typedef': {
			...
			switch(typeObject.type) {
				case 'id': {
					...
					body.push(indent+'{');
					body.push(indent+indent+'if (HyperloopPrivateObjectIsType('+resultname+'Obj,JSPrivateObjectTypeID))');
					body.push(indent+indent+'{');
					body.push(indent+indent+indent+resultname + ' = ('+type+') HyperloopGetPrivateObjectAsID('+resultname+'Obj);');
					body.push(indent+indent+'}');
					body.push(indent+indent+'else if (HyperloopPrivateObjectIsType('+resultname+'Obj,JSPrivateObjectTypeJSBuffer))');
					body.push(indent+indent+'{');
					body.push(indent+indent+indent+'JSBuffer *'+resultname + 'buf = HyperloopGetPrivateObjectAsJSBuffer('+resultname+'Obj);');
					body.push(indent+indent+indent+resultname+' = ('+type+')[NSString stringWithUTF8String:(char*)'+resultname+'buf->buffer];');
					body.push(indent+indent+'}');
					body.push(indent+indent+'else if (HyperloopPrivateObjectIsType('+resultname+'Obj,JSPrivateObjectTypeClass))');
					body.push(indent+indent+'{');
					body.push(indent+indent+indent+resultname+' = ('+type+') HyperloopGetPrivateObjectAsClass('+resultname+'Obj);');
					body.push(indent+indent+'}');
					body.push(indent+indent+'else');
					body.push(indent+indent+'{');
					// turn it into JSON representation if we can't figure it out
					body.push(indent+indent+indent+'JSStringRef '+resultname+'str = JSValueCreateJSONString(ctx,'+varname+',0,0);');
					body.push(indent+indent+indent+'size_t '+resultname+'len = JSStringGetMaximumUTF8CStringSize('+resultname+'str);');
					body.push(indent+indent+indent+'char *'+resultname+'buf = malloc(sizeof(char)*'+resultname+'len);');
					body.push(indent+indent+indent+'size_t '+resultname+'size = JSStringGetUTF8CString('+resultname+'str,'+resultname+'buf,'+resultname+'len);');
					body.push(indent+indent+indent+resultname+'buf['+resultname+"size]='\\0';");
					body.push(indent+indent+indent+resultname+' = ('+type+')[NSString stringWithUTF8String:'+resultname+'buf];');
					body.push(indent+indent+indent+'free('+resultname+'buf);');
					body.push(indent+indent+'}');
					body.push(indent+'}');
					body.push(indent+'else if (JSValueIsString(ctx,'+varname+'))');
					body.push(indent+'{');
					body.push(indent+indent+'JSStringRef '+resultname+'str = JSValueToStringCopy(ctx,'+varname+',0);');
					body.push(indent+indent+'size_t '+resultname+'len = JSStringGetMaximumUTF8CStringSize('+resultname+'str);');
					body.push(indent+indent+'char *'+resultname+'buf = malloc(sizeof(char)*'+resultname+'len);');
					body.push(indent+indent+'size_t '+resultname+'size = JSStringGetUTF8CString('+resultname+'str,'+resultname+'buf,'+resultname+'len);');
					body.push(indent+indent+resultname+'buf['+resultname+"size]='\\0';");
					body.push(indent+indent+resultname+' = ('+type+')[NSString stringWithUTF8String:'+resultname+'buf];');
					body.push(indent+indent+'free('+resultname+'buf);');
					body.push(indent+indent+'JSStringRelease('+resultname+'str);');
					body.push(indent+'}');
					body.push(indent+'else if (JSValueIsBoolean(ctx,'+varname+'))');
					body.push(indent+'{');
					body.push(indent+indent+'bool '+resultname+'num = JSValueToNumber(ctx,'+varname+',0);');
					body.push(indent+indent+resultname+' = ('+type+')[NSNumber numberWithBool:'+resultname+'num];');
					body.push(indent+'}');
					body.push(indent+'else if (JSValueIsNumber(ctx,'+varname+'))');
					body.push(indent+'{');
					body.push(indent+indent+'double '+resultname+'num = JSValueToNumber(ctx,'+varname+',0);');
					body.push(indent+indent+resultname+' = ('+type+')[NSNumber numberWithDouble:'+resultname+'num];');
					body.push(indent+'}');
					body.push(indent+'else if (JSValueIsUndefined(ctx,'+varname+') || JSValueIsNull(ctx,'+varname+'))');
					body.push(indent+'{');
					body.push(indent+indent+resultname+' = ('+type+')[NSNull null];');
					body.push(indent+'}');
					body.push(indent+'else');
					body.push(indent+'{');
					body.push(indent+indent+makeException(indent,'expected object but received something else'));
					body.push(indent+'}');
					return {
						code: body.join('\n'),
						use_pointer: false
...
function generateJSCType(metadata,externs,makers,className,type,subtype,varname,resultname,indent,imports) {
