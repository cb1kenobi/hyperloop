							...
							if (sources.indexOf(source)===-1) {
								...
								if (!obj.is_custom_class) {
									if (!fs.existsSync(source) || genopts.force) {
										fs.writeFileSync(source,typegenerator.generateInterface(state,className,cast));
									}
									if (!fs.existsSync(header) || genopts.force) {
										fs.writeFileSync(header,typegenerator.generateInterfaceHeader(state,className,cast));
									}
									sources.push(source);
								}
							}
