 ...
 * 
 */
function generate (genopts, callback) {
    log.debug('generate');

    buildlib.getSystemFrameworks(function(err,frameworks){
        if (err) return callback(err);
        genopts.state = typegenerator.createState(genopts.metadata,genopts.minversion);
    ...
    buildlib.getSystemFrameworks(function(err,frameworks){
        ...
        try {
            ...
            Object.keys(customclasses).forEach(function(n){
                ...
                if (!fs.existsSync(fh) || genopts.force) {
                    fs.writeFileSync(fh,result.header);
                }
                if (!fs.existsSync(fp) || genopts.force) {
                            ...
                            if (sources.indexOf(source)===-1) {
                                ...
                                if (!obj.is_custom_class) {
                                    if (!fs.existsSync(source) || genopts.force) {
                                        fs.writeFileSync(source,typegenerator.generateInterface(state,className));
                                    }
                                    if (!fs.existsSync(header) || genopts.force) {
                                        fs.writeFileSync(header,typegenerator.generateInterfaceHeader(state,className));
                                    }
                                    sources.push(source);
                                }
                            }
            ...
            var fp = path.join(outdir, CONVERTER_FILENAME.replace('.h','').trim()),
                result = typegenerator.generateInterfaceConverters(state, interfaces, CONVERTER_FILENAME, code, externs);
            ...
            var includedir = path.join(__dirname,'templates','source'),
                hyperloopSource = path.join(includedir,'hyperloop.m'),
                jsbufferSource = path.join(includedir,'JSBuffer.m');

            sources.indexOf(hyperloopSource)===-1 && sources.push(hyperloopSource);
            sources.indexOf(jsbufferSource)===-1 && sources.push(jsbufferSource);
