...
exports.generateInterfacesCode = generateInterfacesCode;
exports.generateProtocolsCode = generateProtocolsCode;
exports.generateSymbolsCode = generateSymbolsCode;
...
function createState (metadata,version) {
	return {
		imports: [],
		makers: [],
		metadata: metadata,
...
function generateInterfacesCode (state, filter) {
	...
	Object.keys(metadata.classes).forEach(function(name) {
		...
		obj.name = name;
		obj._type_ = 'class';
		if (available(obj,version) && obj.framework) {
	...
	return generateInterfaces(state,interfaces);
}

function generateProtocolsCode (state, filter) {
	...
	return generateProtocols(state,protocols);
}

function generateSymbolsCode (state, filter) {
	...
	return generateSymbols(state,symbols);
}

function generateTypesCode (state, filter) {
			...
			case 'typedef': {
				if (primitiveTypeRegex.test(obj.type) && obj.type.indexOf('^')===-1 && obj.type.indexOf('(*)')===-1)
				{
					...
					if (type_names.indexOf(name)===-1) {
						...
						if ((filter && filter(obj,typeObj)) || !filter) {
							types.push(typeObj);
	...
	return generateTypes(state, types);
}

function generateOutput (metadata, version, callback, filter) {
	if (typeof(version)==='function') {
		callback = version;
		version = '7.0';
	}

	var state = createState(metadata,version),
		code = [];

	code.push(generateInterfacesCode(state,filter));
	code.push(generateProtocolsCode(state,filter));
	code.push(generateSymbolsCode(state,filter));
	code.push(generateTypesCode(state,filter));
...
if (module.id===".") {
	...
	// var code = generateInterfacesCode(state,function(obj){return obj.name==='NSString'});
	// var code = generateProtocolsCode(state);
	// var code = generateSymbolsCode(state);
	var code = generateTypesCode(state);
