...
function makeAnyArg (name,value,type) {
	...
	code.push('\t\t}');
	code.push('\t\telse if (HyperloopPrivateObjectIsType(objectRef,JSPrivateObjectTypePointer))');
	code.push('\t\t{');
//	code.push('\t\t\t'+name+' = HyperloopGetPrivateObjectAsPointer(objectRef);');
	code.push('\t\t}');
	code.push('}');
	...
	code.push('{');
	code.push('\tJSStringRef stringRef = JSValueToStringCopy(ctx, '+value+', 0);');
	code.push('\tsize_t buflen = JSStringGetMaximumUTF8CStringSize(stringRef);');
	code.push('\tchar buf[buflen];');
	code.push('\tbuflen = JSStringGetUTF8CString(stringRef, buf, buflen);');
	code.push("\tbuf[buflen] = '\\0';");
	code.push('\t'+name+' = ('+type+')[NSString stringWithUTF8String:buf];');
	code.push('\tJSStringRelease(stringRef);');
	code.push('}');
