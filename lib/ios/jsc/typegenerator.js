...
function makeVarAssignmentFromJSValueRef (state, typeobj, arg, name, argname, cleanup) {
	...
	if (arg.is_pointer_to_pointer) {
		...
		// console.error('is_struct=',arg.is_struct,'pointer=',arg.is_pointer,'varname=',varname,'varassign=',varassign);
...
function generateCustomClass(state,name,classdef,srcdir,srcs,version) {
	...
	copymethods.forEach(function(method) {
		...
		var exCode = [];
		exCode.push('if (exception$ != nil) {');
		exCode.push('  JSObjectRef exObj = JSValueToObject(ctx, exception$, NULL);');
		exCode.push('  JSStringRef exLine = JSStringCreateWithUTF8CString("message");');
		exCode.push('  JSValueRef exProp = JSObjectGetProperty(ctx, exObj, exLine, NULL);');
		exCode.push('  JSStringRelease(exLine);');
		exCode.push('  bool free$2 = false;');
		exCode.push('  NSLog(@"[ERROR] %@", HyperloopJSValueRefToNSString(ctx, exProp, NULL, &free$2));');
		exCode.push('  JSPropertyNameArrayRef exArray = JSObjectCopyPropertyNames(ctx, exObj);');
		exCode.push('  size_t length = JSPropertyNameArrayGetCount(exArray);');
		exCode.push('  for (int i = 0; i < length; i++) {');
		exCode.push('    JSStringRef nameRef = JSPropertyNameArrayGetNameAtIndex(exArray, i);');
		exCode.push('    NSLog(@"[ERROR] %@: %@", [ ( (NSString *) JSStringCopyCFString( kCFAllocatorDefault, nameRef ) ) autorelease ], HyperloopJSValueRefToNSString(ctx, JSObjectGetProperty(ctx, exObj, nameRef, NULL), NULL, &free$2));');
		exCode.push('  }');
		exCode.push('  JSPropertyNameArrayRelease(exArray);');
		if (!returnType.is_void) { exCode.push('  return nil;'); }
		exCode.push('}');

		if (!returnType.is_void) {
			code.push('JSValueRef result$$ = JSObjectCallAsFunction(ctx,fn$,thisObject,'+method.arguments.length+',args,&exception$);');
			code = code.concat(exCode);
    ...
    if (override) {
    	...
    	if (instancetype) {
    		code =  init + '\n' +
    				...
    				'\t' + code.join('\n\t') + '\n' +
    				'}\n' +
    				(!returnType.is_void ? 'return self;' : '');
    	}
    	else {
	    	code = init + ';\n' + code.join('\n');
    	}
...
function generate (metadata, config, callback) {
	...
	var version = config.version,
		...
		state = config.state || createState(metadata,version),
		sources = config.sources || [];

	// return console.error(resolveType(state,'const GLvoid *'));
	// console.error(resolveType(state,'struct CGRect'));
	// console.error(resolveType(state,'fabsf'));
	// return;
	...
	buildlib.getSystemFrameworks(function(err,frameworks){
		...
		// record our system frameworks so we can map them
		state.system_frameworks = frameworks.map(function(m){return m.replace('.framework','').trim()});
		...
		Object.keys(customclasses).forEach(function(n){
	    	var result = generateCustomClass(state,n,customclasses[n],outdir,sources,version),
	    		fh = path.join(outdir, 'js_custom_'+n+'.h'),
	    		fp = path.join(outdir, 'js_custom_'+n+'.m');
			...
			var type = resolveType(state,n);
			type.object.import = '"js_custom_'+n+'.h"';
			...
			switch (obj.metatype) {
				...
				case 'symbol': {
					if (symbols.indexOf(obj.mangledName)===-1) {
						...
						if (!type) {
							console.error("Can't resolve:",obj.object.type);
							process.exit(1);
						}
		...
		var fp = path.join(outdir, CONVERTER_FILENAME.replace('.h','').trim()),
			result = generateInterfaceConverters(state, interfaces, CONVERTER_FILENAME, code, externs);
