...
function makeBlock(state,object) {
	...
	code.push('\tdispatch_sync(dispatch_get_main_queue(), ^{');
	if (blockArgs.length && !isVoid) {
		code.push('\t\tJSValueRef args['+blockArgs.length+'];');
		object.blockArgs.forEach(function(arg){
			var body = convertToJSValueRef(state, object, arg.mangledName, arg, argnames[count]);
			code.push('\t\t'+body);
			code.push('\t\targs['+(count++)+'] = '+argnames[count-1]+';');
		});
	}
	else {
		code.push('\t\tJSValueRef args[0];');
		argCount = 0;
	}
	code.push('\t\tJSValueRef exception$ = NULL;');
	if (!blockReturn.is_void) {
		code.push('\t\tJSValueRef fnResult$ = JSObjectCallAsFunction(ctx,function,thisValue,'+argCount+',args,&exception$);');
	}
	else {
		code.push('\t\tJSObjectCallAsFunction(ctx,function,thisValue,'+argCount+',args,&exception$);');
	}
	...
	code.push('};');
	code.push('return [Block_copy(block$) autorelease];');
