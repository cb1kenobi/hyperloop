...
function resolveType (state, type) {
	...
	var result = {
		metatype: is_enum ? 'enum' : is_struct ? 'struct' : classObject ? 'interface' : protoObject ? 'protocol' : typedObject ? 'typedef' : symbolObject ? 'symbol' : is_primitive ? 'primitive' : is_protocol ? 'protocol' : is_function_pointer ? 'function_pointer' : is_block ? 'block' : 'other',
		realtype: type,
		type: name,
		name: name,
...
function makeMethod (state, typeobj, methodName, resultName, instanceName, argumentsName, argumentsLengthName) {
	// return 'return JSValueMakeUndefined(ctx);';
...
function generateInterfaceConverters (state, interfaces) {
	...
	interfaces.forEach(function(entry) {
		var framework = entry && entry.framework;
		if (!framework) {
			console.error("Couldn't find framework for interface:",name);
			process.exit(1);
		}
...
if (module.id===".") {
	var json = "/Users/jhaynie/Library/Application Support/org.appcelerator.hyperloop/cache/bbf2d450bac4ef5e4c37c2fc264c7e23.json",
		...
		//state = createState(metadata,'7.0',{'UIView':{methods:['constraints','insertSubview','setBounds','setBackgroundColor'],properties:['bounds']}});
		state = createState(metadata,'7.0');

	log.debugLevel = true;

	// return console.log(resolveType(state,'NSRange'));
	// return console.log(resolveType(state,'struct CGContext *'));
	// return console.log(resolveType(state,'const unichar *'));
	// return console.log(resolveType(state,'CGPathRef'));
	// return console.log(resolveType(state,'struct CGPath *'));
	// return console.log(resolveType(state,'Class<UIAppearanceContainer> *'));

	resolveType(state, 'UIView');

	var findSymbol;
	// var findSymbol = 'UIBezierPath';
	// var findSymbol = 'NSString';
	// var findSymbol = 'MPMediaPlayback';
	// var findSymbol = 'UILayoutSupport';
	// var findSymbol = 'MPMoviePlayerViewController';
	// var findSymbol = 'UIViewController';
	// var findSymbol = 'UIViewControllerRestoration';
	// var findSymbol = 'OS_dispatch_queue';
	// var findSymbol = 'UIImageView';
	// var findSymbol = 'NSMutableString';
	// var findSymbol = 'NSNumber';
	// var findSymbol = 'NSLayoutManager';
	// var findSymbol = 'UIAppearanceContainer';
	// var findSymbol = 'CIImage';
	// var findSymbol = 'UIGestureRecognizer';
	// var findSymbol = 'UIView';
	...
	_.compact(Object.keys(state.dependencies).sort()).forEach(function(symbolName) {
		...
		//console.log(symbolName,'=>',obj.metatype);
		switch (obj.metatype) {
			case 'interface': {
			...
			case 'interface': {
				var className = obj.name,
					...
					source = fp+'.m',
					header = fp+'.h';
				if (findSymbol && symbolName!==findSymbol) {
					return;
				}
			...
			case 'struct': {
				var simpleType = 'js_'+mangleTypeName(symbolName),
					fn = (obj && obj.object && obj.object.framework ? obj.object.framework+'/' : '') + simpleType,
			...
			case 'typedef': {
				var className = obj.name,
					fn = 'js_'+mangleTypeName(className),
					fn = (obj && obj.object && obj.object.framework ? obj.object.framework+'/' : '') + fn,
					fp = path.join(outdir, fn),
					source = fp+'.m',
					header = fp+'.h';
				if (sources.indexOf(source)===-1) {
