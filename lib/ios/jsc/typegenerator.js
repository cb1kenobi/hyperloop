	...
	if (result.is_object) {
		...
		result.object.methods && Object.keys(result.object.methods).forEach(function(name) {
			...
			methods.forEach(function(method) {
				...
				if (method.returnSubtype==='instancetype') {
					m.returnType = resolveType(state, simpleType);
				}
				var args = [];
...
function generate (metadata, config, callback) {
	...
	// resolveType(state,'const CLLocationAccuracy');
    // return console.error(Object.keys(state.dependencies).sort());
		...
		switch (obj.metatype) {
			...
			case 'symbol': {
				if (symbols.indexOf(obj.mangledName)===-1) {
					...
					var symcode = [];
					type.object && type.framework && (symcode.push('@import '+type.object.framework+';'));
					obj.object.framework && (symcode.push('@import '+obj.object.framework+';'));
