...
function makeBlock(state,object) {
	...
	if (!blockReturn.is_void) {
		blockcode.push('\t\tJSValueRef fnResult$ = JSObjectCallAsFunction(ctx,function,thisObject,'+argCount+',args,&exception$);');
	}
	else {
		blockcode.push('\t\tJSObjectCallAsFunction(ctx,function,thisObject,'+argCount+',args,&exception$);');
	}
