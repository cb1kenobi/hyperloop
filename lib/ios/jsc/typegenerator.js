	...
	if (typeobj.object.formatter && args.length) {
		...
		for (var c=1;c<5;c++) {
			...
			for (var x=1;x<c;x++) {
				...
				body.push('\t\t'+util.indentify(makeAnyArg(argname,argname+'$value',anyType),'\t\t'));
			}
...
function makeMethod (state, typeobj, methodName, resultName, instanceName, argumentsName, argumentsLengthName, returnFilterFunc, useSuperCheck) {
	...
	_.compact(methods).forEach(function(method) {
		...
		argbody.forEach(function(b){
			code.push(indent+util.indentify(b,indent));
		});
		fnbody && code.push(indent+fnbody);
		code.push(indent+util.indentify(returnbody,indent));
		if (useSuperCheck) {
		...
		cleanup.length && (code.push(indent+util.indentify(cleanup.join('\n'),indent)));
...
function generateInterface (state, name) {
	...
	return ejs.render(class_template, util.mixin({
...
function generateStructHeader(state, name) {
	...
	return ejs.render(struct_header_template, util.mixin({
		state: state,
		entry: obj.object,
		object: obj,
...
function generateStruct (state, name) {
	...
	return ejs.render(struct_template, util.mixin({
		state: state,
		entry: obj.object || {},
		object: obj,
		instanceName: instanceName,
...
function generateTypedef (state, name) {
	...
	return ejs.render(typedef_template, util.mixin({
		state: state,
		entry: obj.object,
		object: obj,
...
function generateBlock(state, object) {
	...
	return ejs.render(block_to_native_template, util.mixin({
		state:state,
		object:object,
		imports: imports,
		externs: externs,
...
function generateFunctionPointer(state, object) {
	...
	if (!skip) {
		...
		if (object.functionPointerReturnType.is_void) {
			...
			callback.push('\tCHECK_EXCEPTION(hfp.ctx,exception);');
	...
	return ejs.render(function_pointer_to_native_template, util.mixin({
		state:state,
		object:object,
		imports: imports,
		externs: _.uniq(externs),
...
function generateFunction(state, object) {
	...
	return ejs.render(function_to_native_template, util.mixin({
		state:state,
		object:object,
...
function generateCustomClass(state,name,classdef,srcdir,srcs,version) {
	...
	var implementation = ejs.render(custom_class_template, util.mixin({
		methods: methods,
	...
	var header = ejs.render(custom_class_header_template, util.mixin({
		methods: methods,
