			...
			case 'typedef': {
				if (primitiveTypeRegex.test(obj.type) && obj.type.indexOf('^')===-1 && obj.type.indexOf('(*)')===-1)
				{
					...
					if (type_names.indexOf(name)===-1) {
						...
						var obj = {_type_:'typedef',isCharArray:isCharArray,length:length,simpleType:simpleType,varType:varType,returnType:returnType,obj:obj,name:name,isArray:isArray,isPointerToPointer:isPointerToPointer,isPointer:isPointer};
						obj.defaultValue = defaultValue(obj);
						types.push(obj);
						//console.log(name,'=>',JSON.stringify(types[types.length-1]))
					}
				}
			}
		}
