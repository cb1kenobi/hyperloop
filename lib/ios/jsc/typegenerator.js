...
function makeBlock(state,object) {
	...
	if (!blockReturn.is_void) {
		blockcode.push('\t\tJSValueRef fnResult$ = JSObjectCallAsFunction(ctx,function,thisValue,'+argCount+',args,&exception$);');
	}
	else {
		blockcode.push('\t\tJSObjectCallAsFunction(ctx,function,thisValue,'+argCount+',args,&exception$);');
	}
