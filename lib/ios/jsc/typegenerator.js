...
function resolveType (state, type) {
	...
	else if (is_struct && typeObject.fields)
	{
...
function compile (metadata, config, callback) {
	...
	// var fn = makeFunction(state,resolveType(state,'EAGLGetVersion'),'arguments', 'argumentCount');
...
//return console.log(symbols)
	...
	// push coverter code
	code.push(generateInterfaceConverters(state, interfaces, CONVERTER_FILENAME));

	// now do convertors code
	var fp = path.join(outdir, CONVERTER_FILENAME.replace('.h','').trim());
	fs.writeFileSync(fp+'.m', code.join('\n'));
...
if (module.id===".") {
	...
	var config = {
		version: '7.0',
		types: ['UIView','EAGLGetVersion','CGPathCreateMutable'],
		builddir: builddir,
		outdir: outdir
	};
