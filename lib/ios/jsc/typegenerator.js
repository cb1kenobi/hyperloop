...
function generateCustomClass(state,name,classdef,srcdir,srcs,version) {
	...
	copymethods.forEach(function(method) {
		...
		cleanup.length && (code=code.concat(cleanup));
		if (!returnType.is_void) {
			code.push('JSObjectRef selfObject = ' + (instancetype ? '(struct OpaqueJSValue *)Hyperloop' + name + 'ToJSValueRef(ctx,self)' : 'NULL') + ';');
			code.push('JSValueRef result$$ = JSObjectCallAsFunction(ctx,fn$,selfObject,'+method.arguments.length+',args,&exception$);');
			...
			code.push('JSObjectCallAsFunction(ctx,fn$,NULL,'+method.arguments.length+',args,&exception$);');
		}
...
function generate (metadata, config, callback) {
	...
	// console.error(resolveType(state,'union _GLKVector4'));
	// return;
