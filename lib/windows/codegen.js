...
WinCodegen.prototype.generate = function(asts, generateASTCallback, callback) {
	...
	if (cacheFile && fs.existsSync(cacheFile)) {
		...
		return generateImports(options, metabase, asts, callback);
	}
	...
	programs.ildasm(ref, 'windows.il', function(err, ref) {
		...
		ilparser.parseFile(ref, function(err, ast) {
			...
			generateImports(options, metabase, asts, callback);
		});
	});

	function generateImports(options, metabase, asts, callback) {
		var left = asts.length;

		function done() {
			left -= 1;
			if (left === 0) {
				generateWithMetaBase(metabase, asts, callback);
			}
		}
	...
	function generateWithMetaBase(metabase, asts, callback) {
		...
		for (var i = 0; i < asts.length; i++) {
			...
			// Build up our referenceTable.
			if (ast.ast) {
				ast.ast.transform(new Uglify.TreeTransformer(null, function(node, descend) {
					var extractFrom;
					if (node instanceof Uglify.AST_Call) {
						extractFrom = node.expression.property;
					}
					else if (node instanceof Uglify.AST_New) {
						extractFrom = node.expression;
					}
					else if (node instanceof Uglify.AST_PropAccess) {
						extractFrom = node.property;
					}
					...
					if (extractFrom) {
						var key = '' + (extractFrom.name || extractFrom);
						if (key !== '[object Object]') {
							referenceTable[key] = true;
						}
					}
				}));
			}
		...
		Object.keys(generate.sourceResults).forEach(function(key) {
			generate.sourceResults[key] = generate.sourceResults[key]
				.replace(/"/g, '\\"') // Escape quotes.
				.replace(/\\(n)/g, '\\\\' + 'n') // Replace '\n' string with a more escaped new line.
				.replace(/\n/g, '\\\n'); // Escape the end of lines so the string will continue on.
		});
