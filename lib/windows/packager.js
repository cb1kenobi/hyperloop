	...
	if (version!==jscVersion) {
		...
		req.on('response', function(res) {
			...
			var len = parseInt(res.headers['content-length'], 10),
				bar = new appc.progress('  Downloading JavaScriptCore library'.magenta+' [:bar]'+' :percent :etas'.cyan, {
				complete: '=',
				incomplete: ' ',
				width: 50,
				total: len
			}),
			jsf = path.join(jscDir,'JavaScriptCore.zip'),
			...
			res.on('end', function() {
				...
				appc.zip.unzip(jsf,jscDir,function(err){
					log.debug('unzip completed');
					fs.unlink(jsf,callback);
				});
			});
		});
