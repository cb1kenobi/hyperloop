...
function parseGetOrSetToken(prop, node) {
	...
	if (!m) {
		console.error(prop+" regex failed");
...
function parseFieldToken(node) {
	...
	if (!m) {
		console.error("couldn't parse field token");
		console.error(node.line);
		process.exit(1);
	}
	var obj = {};
	...
	args.forEach(function(arg) {
		if (arg) {
			if (!methodSignatureRegex.test(arg)) {
				...
				console.error('sigline=',sigline);
				console.error('tok=',tok);
				console.error('arg=',arg);
				console.error('functionName=',functionName);
				console.error('returnType=',returnType);
				console.error('attributes=',obj.attributes);
				console.error('subtoks=',subtoks);
				for (var c=1;c<tok.length;c++) {
					var token = tok[c];
...
function parseNode(node,indent) {
	// console.log(indent+'parseNode',node.metatype,'\t\t\t\t'+(node.line && node.line.substring(0,Math.min(20,node.line.length))));
	...
	if (node.children) {
		...
		node.children.forEach(function(child){
			//console.log(indent+'parsing child node',child.metatype,'=>',child.line);
	...
	for (var c=0;c<buffers.length;c++) {
		...
		if (isCommandStart) {
			...
			if (blockStarted) {
				...
				console.error("parser error. should haven't gotten here. not sure what to do with this line:");
				console.error(line);
				process.exit(1);
			}
		}
...
if (module.id === ".") {
	...
	if (!fn) {
		console.error("specify a file to the output of idladm");
	}
	parseFile(fn, function(err, ast, json) {
		console.log(JSON.stringify(ast.toJSON(),null,3));
	});
}
