...
function mangleTypeName(type) {
	return type.split('.').pop().replace(/[`\(\)\[\]\s,<>\.]/g, '_').replace(/\^/g, 'B').replace(/\*/g, 'P');
}
...
function resolveObject(state, result) {
	...
	result.object.methods && Object.keys(result.object.methods).forEach(function(ii) {
		var method = result.object.methods[ii],
			name = method.name && method.name.replace('.', '_');

		if (method.name === '.ctor' || !name) {
			return;
		}
		if (method.returnType.indexOf('<') >= 0) {
			// TODO: Handle generics.
			// console.log('Skipping ' + method.name + ' because of ' + method.returnType + '!');
			return;
		}
		...
		if (method.args && method.args.length) {
			method.args.forEach(function(a) {
				...
				if (a.type.indexOf('<') >= 0) {
					// TODO: Handle generics.
					// console.log('Skipping ' + m + ' because of ' + arg.type + '!');
					skip = true;
					return;
				}
				var rtc = _.clone(rt);
		...
		if (!skip) {
			m.args = args;
			if (method.attributes && method.attributes.indexOf('instance') >= 0) {
	...
	result.object.properties && Object.keys(result.object.properties).forEach(function(name) {
		...
		if (property.getter && property.getter.classname && property.getter.classname.indexOf('<') >= 0) {
			// TODO: Handle generics.
			// console.log('Skipping ' + name + ' because of ' + property.getter.classname + '!');
			return;
		}
...
function makeMethod(state, typeobj, methodName, resultName, instanceName, argumentsName, argumentsLengthName, returnFilterFunc) {
	...
	_.compact(methods).forEach(function(method) {
		...
		if (has_args) {
			var arg0 = method.args[0],
				argName = (arg0.typeObject && arg0.typeObject.name) || (arg0.argument && arg0.argument.name) || arg0.name,
				varAssign = makeVarAssignmentFromJSValueRef(state, typeobj, arg0.typeObject || arg0, argName + '$' + 0, argumentsName + '[0]', cleanup);
...
function makeBlock(state, object) {
	...
	var code = [],
		argnames = [],
		blockArgs = object.blockArgs.map(function(a) {
			var n = 'arg' + argnames.length;
			argnames.push(n);
			return a.type + ' ' + n + '$'
		}),
	...
	blockcode.push('\t\tCHECK_EXCEPTION(ctx,exception$);');
	if (!blockReturn.is_void) {
		var cleanup = [];
...
function generateInterface(state, name) {
	...
	return util.renderTemplate('templates/class.ejs', {
		...
		object: obj,
		instanceName: obj.instanceName,
		fullInstanceName: obj.className.replace(/\./g, '::'),
		name: name,
...
function generateCast(state, name, cast) {
	var obj = resolveType(state, cast.argType),
		templateArgs = {
			state: state,
			entry: obj.object || {},
			functionName: cast.functionName,
			object: obj,
...
function generateCustomClass(state, name, classdef, srcdir, srcs, version) {
	...
	Object.keys(typeobj.class_methods).sort().forEach(function(m) {
		...
		// Skip methods with generics in them; we'll support them later!
		var args = classMethod[0].args;
		...
		var code = makeMethod(state, typeobj, classMethod, 'result', 'instance', 'arguments', 'argumentCount', function(resultname, instancename) {
 ...
 * @param {...1} var_args Any additional namespaces to also add, such as the class's namespace itself.
 * @returns {*}
 */
function convertToUsings(includes) {
