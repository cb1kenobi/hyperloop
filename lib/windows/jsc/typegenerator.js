 ...
 */
function mangleTypeName(type, preserveSymbols) {
	var reducedType = type.match(/^[^<]+\.([^.]+)/);
	var retVal = (reducedType ? reducedType[1] : type)
		.replace('[]', 'Array')
		.replace(/[<>]/g, '$')
		.replace(/[`\(\)\[\]\s,\.]/g, '_')
		.replace(/\^/g, '')
...
function resolveType(state, type) {
	...
	var originalType = type.trim(),
		...
		reducedType = !is_generic ? originalType.match(/.+\.([^.]+)/) : originalType.match(/[^<]+\.([^>]+>)/);
	type = (reducedType ? reducedType[1] : originalType).replace(/[&^]/g, '');
	...
	if (is_generic) {
		...
		for (var key in state.generics) {
			if (state.generics.hasOwnProperty(key)) {
				if (state.generics[key].classname === matchingClassName) {
...
function convertToJSValueRef(state, typeobj, mangledName, arg, resultName) {
	...
	if (!arg) {
		return '// TODO: Passed in null arg.';
	}
...
function convertToUsings(includes, var_args) {
	var retVal = _.uniq(includes.map(function(imp) {
		return imp.indexOf('<') > 0 ? false : imp.split('.').slice(0, -1).join('::');
	}).filter(truthy));
