...
function resolveObject(state, result) {
	...
	if (simpleType.indexOf('IAsync') >= 0) {
	...
	result.object.methods && Object.keys(result.object.methods).forEach(function(ii) {
		...
		if (method.attributes && method.attributes.indexOf('public') === -1 && !state.handlerBindings[result.mangledName]) {
			return;
		}
...
function resolveType(state, type) {
	...
	if (matchingClassNames.length > 1) {
		matchingClassNames = disambiguate(type, state.generate.compiler, matchingClassNames);
	}
	var includes = [],
	...
	if (is_generic) {
		...
		if (type.split('<').length > 2) {
			log.trace('Skipping ' + type + ' because nested generics are not yet supported.');
			return null;
		}
	...
	if (is_generic) {
		...
		mangled = type.split('`')[0] + '$' + (generic_types.map(function(a) {
			a.className && includes.push(a.className);
		...
		if (has_args) {
			...
			for (var c = 0; c < method.args.length; c++) {
				...
				var isIVector = method.overrides && method.overrides.type && method.overrides.type.indexOf('IVector') >= 0,
					isToArray = method.name.slice(-5) === 'Array' && arg.type.slice(-1) === '^';
		...
		if (!is_void && !method.requiresSentinel) {
			...
			if (!returnbody) {
				...
				if (returnType !== 'void' && returnType.metatype !== 'primitive' && returnType.className) {
					includes.push(returnType.fullInstanceName);
				}
...
function disambiguate(type, compiler, matches) {
	...
	log.error('' + type.bold + ' is ambiguous between ' + matches.join(' and ') + '!');
	return null;
}
