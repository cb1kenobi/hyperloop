	...
	_.compact(methods).forEach(function(method) {
		...
		if (has_args) {
			for (var d = 0; d < method.args.length; d++) {
				...
				if (varg2 && varg2.inout === 'out' && varg2.type.indexOf('valuetype ') < 0) {
					code.push(indent + 'reinterpret_cast<JSPrivateObject*>(JSObjectGetPrivate(JSValueToObject(ctx, arguments[' + d + '], exception)))->object = ' + varg2.name + '$' + d + ';');
				}
			}
		}
