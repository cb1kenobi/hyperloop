...
WinLauncher.prototype.launch = function(options, args, callback) {
	...
	programs.msbuild('"' + paths.solutionFile + '" /p:Platform="Win32";Configuration=' + options.configuration, function didBuild(err) {
		if (err) {
	...
	function findPackagePs1() {
		...
		programs.powershell('"(Get-WindowsDeveloperLicense | Where-Object { $_.IsValid }).Count -eq 0"', function(err) {
			if (err) {
				log.error('Failed to search for developer license.');
				log.fatal(err);
			}

			var cmd = '-ExecutionPolicy Unrestricted "& \'' + path.join(appPackages, files[0]) + '\'' +
				(needLicense ? '' : ' -Force') + '"';
			programs.powershell(cmd, function(err) {
				if (err) {
					log.error('Failed to install the app.');
