...
exports.run = function(options, headers, sourcefile, callback) {
	var paths = Paths.fetch(options),
		...
		remaining = headers.length,
		mixed = {};
	// Let's get started!

	//return remaining && proceedSync();
	return remaining && proceedAsync();
	...
	function proceedSync() {
		...
		var h = headers[headers.length - remaining], // Parse headers in order.
			found = findHeader(h);
		parseHeader(options, headerSearchPaths, h, found, headerReadySync);
	}

	function headerReadySync(ast) {
		mixInAST(ast, mixed);
		if (--remaining === 0) {
			log.debug('All headers have been parsed!');
			allDone();
		}
		else {
	...
	function proceedAsync() {
		for (var i = 0, iL = headers.length; i < iL; i++) {
			var h = headers[i],
				found = findHeader(h);
			parseHeader(options, headerSearchPaths, h, found, headerReadyAsync);
		}
	}
...
function parseHeader(options, headerSearchPaths, header, headerPath, parsed) {
	...
	// Generate a checksum based on the arguments; we want it to be unique.
		...
		stdOut = path.join(process.env.TEMP, clangOutputChecksum + '.out.txt'),
		stdErr = path.join(process.env.TEMP, clangOutputChecksum + '.err.txt'),
		resultCache = path.join(process.env.TEMP, parsedChecksum + '.ast.json');

	if (fs.existsSync(resultCache)) {
