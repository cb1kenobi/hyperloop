...
function compileIfNecessary(cp, callback) {
	...
	if (fs.existsSync(classFile)) {
		...
		var process = spawn('javac',['-source','1.6','-target','1.6','-cp',cp,path.join(__dirname,'JavaMetabaseGenerator.java'),'-d',outdir],{env:process.env}),
			err = '';
...
function generate(classPath, callback) {
	...
	compileIfNecessary(cp, function(err){
		...
		var process = spawn('java',['-Xmx1G','-classpath',cp,'JavaMetabaseGenerator']),
			out = '',
			err = '';
