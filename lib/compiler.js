...
const KEYWORDS = ['import','compiler','class','memory','owner'];
const RESERVED_SYMBOLS = new RegExp('^hyperloop_('+KEYWORDS.join('|')+')$');
...
function compile (source, filename, sourcefile) {
	...
	var transfomer = new Uglify.TreeTransformer(null, function(node,descend){
		...
		if (node instanceof Uglify.AST_SimpleStatement) {
			...
			else if (node.expression && node.expression.name=='hyperloop_owner') {
				if (node.args.length!=2) {
					console.error('Invalid @owner usage. Must supply 2 arguments as JS object, native object at '+node.start.file+' on '+node.start.line);
					process.exit(1);
				}
				if (!node.args[0].name) {
					console.error('Invalid @owner usage. first argument must be JS variable name at '+node.start.file+' on '+node.start.line);
					process.exit(1);
				}
				if (!node.args[1].name) {
					console.error('Invalid @owner usage. second argument must be JS variable name at '+node.start.file+' on '+node.start.line);
					process.exit(1);
				}
				var jsname = node.args[0].name,
					nativename = node.args[1].name;
				sourcefile.processOwner(node,jsname,nativename);
				return new Uglify.AST_SymbolConst({name: 'HL$TrackOwner('+jsname+','+nativename+')'});
			}
